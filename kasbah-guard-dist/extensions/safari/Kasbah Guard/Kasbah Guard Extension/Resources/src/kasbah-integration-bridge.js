 ...
 *     ↓
 *   kasbah-frontier-image.js (GenAI text + 8-method image AI detection)
 *     ↓
 *   zchat1 bridge (orchestration)
 *     ├→ Frontier Text Analysis (entropy + phrase markers)
 *     ├→ Image AI Detection (8-method ensemble — local, no network)
 ...
 *     ↓
 *   Enhanced verdict (base + all analyses)
 *
...
class KasbahIntegrationBridge {
  constructor(config = {}) {
    ...
    this.modulesAvailable = {
      ...
      calibration: false,
      ethics: false,
      frontier: true,   // always available — pure JS, no network
      imageAI: true     // always available — pure JS, no network
    };
  ...
  async enhanceDetectionResult(content, baseDetectionResult) {
    ...
    try {
      ...
      // 0a. Frontier GenAI text analysis (local, no network)
      if (content.data && typeof content.data === 'string') {
        try {
          const frontier = window.kasbahFrontier;
          if (frontier) {
            analyses.frontier = frontier.analyzeText(content.data);
          }
        } catch (error) {
          analyses.errors.push({ module: 'frontier', error: error.message });
        }
      }

      // 0b. Image AI detection (local canvas-based, no network)
      if (content.element && window.kasbahFrontier) {
        try {
          analyses.imageAI = window.kasbahFrontier.analyzeImageElement(content.element);
        } catch (error) {
          analyses.errors.push({ module: 'imageAI', error: error.message });
        }
      }
  ...
  combineAnalyses(baseResult, analyses) {
    // Apply Frontier score boost: if GenAI detected with high confidence,
    // bump risk/confidence (same semantics as VS Code extension detector.ts)
    let boostedConfidence = analyses.calibration || baseResult.confidence;
    let frontierAnnotation = null;

    if (analyses.frontier && analyses.frontier.genai) {
      frontierAnnotation = {
        genai: true,
        generator: analyses.frontier.generator,
        confidence: analyses.frontier.confidence,
        entropy: analyses.frontier.entropy
      };
      if (analyses.frontier.confidence > 0.8 && baseResult.risk !== undefined) {
        // Small risk boost (max +5) — mirrors VS Code extension behaviour
        baseResult = { ...baseResult, risk: Math.min(100, (baseResult.risk || 0) + 5) };
      }
    }

    // Apply Image AI result: if AI-generated image detected, escalate confidence
    let imageAIAnnotation = null;
    if (analyses.imageAI && analyses.imageAI.isAiGenerated) {
      imageAIAnnotation = {
        isAiGenerated: true,
        aiScore: analyses.imageAI.aiScore,
        generatorHint: analyses.imageAI.generatorHint,
        confidence: analyses.imageAI.confidence,
        dominantArtifacts: analyses.imageAI.dominantArtifacts,
        methodScores: analyses.imageAI.methodScores
      };
      // Escalate calibrated confidence if image AI score is high
      if (analyses.imageAI.aiScore > 0.7) {
        boostedConfidence = Math.max(boostedConfidence || 0, analyses.imageAI.aiScore);
      }
    }
    ...
    const enhanced = {
      ...baseResult,
      zchat1: {
        frontier: frontierAnnotation,
        imageAI: imageAIAnnotation,
        spatial: analyses.spatial,
        generator: analyses.generator,
        calibration: {
          raw: baseResult.confidence,
          calibrated: boostedConfidence
        },
  ...
  /**
   * Allow kasbah-frontier-image.js to register itself (defensive patch)
   */
  _patchFrontier() {
    this.modulesAvailable.frontier = true;
    this.modulesAvailable.imageAI  = true;
    console.log('[Kasbah] Frontier+Image detection registered.');
  }
