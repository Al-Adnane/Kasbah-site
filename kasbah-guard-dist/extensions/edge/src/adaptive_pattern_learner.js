...
class AdaptivePatternLearner {
   ...
   * Heuristic validation — no external negative corpus
   * False positive rate = (FP) / (FP + TN) against a fixed local sample set.
   */
   ...
   * Local consensus only — remote federation pending
   * Returns count of local detections; no network calls are made.
   */
   ...
   * federation not yet active — stub only, no network calls are made.
   * Users can opt-in to share via telemetry once federation is implemented.
   */
  async broadcastPatternUpdate(pattern) {
    // federation not yet active — stub only
    console.debug('[AdaptivePatternLearner] broadcastPatternUpdate: federation not yet active');
  }
