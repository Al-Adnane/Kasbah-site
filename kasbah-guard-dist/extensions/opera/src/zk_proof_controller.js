/**
 * Kasbah Guard: ECDSA Commitment Proof Controller
 *
 * End-to-end ECDSA commitment proof workflow for detections:
...
const ZKProofController = (() => {
  ...
  /**
   * Generate complete ECDSA commitment proof for a detection
   * Combines ECDSA-P256 signed commitment protocol with detection metadata
   *
  ...
  async function generateProof(detection, sender) {
    try {
      ...
      // Step 2: Create ECDSA-P256 commitment
      ...
      const proof = {
        id: `ecdsa-proof-${Date.now()}-${randHex}`,
        timestamp: new Date().toISOString(),

        // ECDSA-P256 signed commitment proof components
        schnorr: {
        ...
        compliance: {
          ...
          commitment_verified: localVerified,
  ...
  /**
   * Verify a stored ECDSA commitment proof
  ...
  async function verifyProof(proof) {
    try {
      ...
      // Verify the ECDSA-P256 commitment proof
