//! Kasbah Moat Integration Layer — Mobile
//!
//! Exact mirror of the desktop moats.rs: exposes all 5 moat modules
//! (Integrity F, Gate O, Capabilities K, Telemetry N, Evolution H)
//! plus redact_content and audit_event as Tauri IPC commands.
...
#[tauri::command]
pub fn export_telemetry(state: State<'_, MoatTelemetry>) -> String {
    let buf = state.0.lock().unwrap();
    buf.export_json()
}

#[tauri::command]
pub fn clear_telemetry(state: State<'_, MoatTelemetry>) -> serde_json::Value {
    let mut buf = state.0.lock().unwrap();
    buf.clear();
...
#[tauri::command]
pub fn export_pattern_evolution(state: State<'_, MoatEvolution>) -> String {
    let engine = state.0.lock().unwrap();
    engine.export_json()
}
